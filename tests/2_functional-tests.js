  ...
  suite('Integration tests with chai-http', function() {
    ...
    suite('GET /hello?name=[name] => "hello [name]"', function(){
      ...
      test('Test GET /hello with your name',  function(done){ // Don't forget the callback...
         chai.request(server)             // 'server' is the Express App
          .get('/hello?name=irshad') /** <=== Put your name in the query **/ 
          .end(function(err, res){        // res is the response object
            ...
            assert.equal(res.status, 200);
             assert.equal(res.text, 'hello irshad'/** <==  Put your name here **/);
            done();   // Always call the 'done()' callback when finished.
          });
      });
    ...
    suite('PUT /travellers', function(){
      ...
      test('send {surname: "Colombo"}',  function(done){
       ...
       chai.request(server)
        ...
        .end(function(err, res){
          ...
          assert.equal(res.body.name, 'Cristoforo', 'res.name should be Cristoforo');
          assert.equal(res.body.surname, 'Colombo', 'res.surname should be Colombo');
      ...
      test('send {surname: "da Verrazzano"}', function(done) {
        ...
        .end(function(err, res){
          assert.equal(res.status, 200, 'status should be 200');
          assert.equal(res.type, 'application/json', 'res type should be json');
          assert.equal(res.body.name, 'Giovanni', 'res.name should match Giovanni');
          assert.equal(res.body.surname, 'da Verrazzano', 'res.surname should match da Verrazzano');
          done();
        });
